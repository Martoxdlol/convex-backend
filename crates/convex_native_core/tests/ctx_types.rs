...
async fn _widget_query<RT: common::runtime::Runtime>(ctx: &mut QueryCtx<'_, RT>) {
    ...
    let with_meta: Option<convex_native_core::DocumentWithMeta<Widget>> =
    ...
    let first_page: convex_native_core::TypedPage<Widget> = ctx
        .db()
        .query::<Widget>()
        .with_index(WidgetIndex::ByOwner)
        .eq(WidgetField::Owner, "alice".to_string())
        ...
        .await
        .unwrap();
    let _next: Option<convex_native_core::Cursor> = first_page.cursor;
